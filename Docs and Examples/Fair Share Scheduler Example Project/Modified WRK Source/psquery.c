...

{
    ...
    case ThreadSetScheduleBucket:
        ...
        if ((ULONG)ScheduleBucket >= MAXIMUM_SCHEDULE_BUCKETS)
            return STATUS_INVALID_PARAMETER;

        //
        // Operates only on the current thread to guarantee that threads
        // are never put into a scheduling bucket while on the ready list
        // as that would screw up the counters.
        //
