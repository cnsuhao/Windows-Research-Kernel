...
THIS IS NOT THE ORIGINAL WRK SOURCE FROM MICROSOFT.  IT HAS BEEN MODIFIED.
Modified by DavePr@Microsoft.com - October 2006
...
struct {
    ULONG ReadyGroups;                          // N
    ULONG ReadyThreads;                         // M
    ULONG Buckets[MAXIMUM_SCHEDULE_BUCKETS];    // n[i]
} BucketScheduling;
...

{
    ...
    if ((Thread->Affinity & Prcb->SetMember) != 0) {
        ...
        //
        // Keep track of buckets for threads on ReadyList
        //
...

{
    ...
    if (PrioritySet != 0) {
        ...
        if (eThread->BucketsEnabled) {
            ...
            newquantum = Thread->QuantumReset * BucketScheduling.ReadyThreads / (BucketScheduling.ReadyGroups * BucketScheduling.Buckets[bucket]);
            if (newquantum > 127)
                newquantum = 127;
